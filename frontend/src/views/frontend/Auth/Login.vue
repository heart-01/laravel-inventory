          ...
          <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
            <div class="w-full">
              ...
              <form @submit.prevent="onSubmit" >
                ...
                <input v-model="formLogin.email" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight" type="text" autocomplete="email" >
                <div v-if="v$.formLogin.email.$error" class="mt-2 text-sm text-red-500"> {{ v$.formLogin.email.$errors[0].$message }}</div>

                <label class="block text-gray-700 text-sm mt-3 mb-2" for="password">รหัสผ่าน</label>
                <input v-model="formLogin.password" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight" type="password" autocomplete="current-password">
              ...
              <button class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium leading-5  transition-colors duration-150 border border-gray-300 rounded-lg dark:text-gray-400 active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray">
              ...
              <button class="flex items-center justify-center w-full px-4 py-2 mt-4 text-sm font-medium leading-5 transition-colors duration-150 border border-gray-300 rounded-lg dark:text-gray-400 active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray">
                <svg class="w-4 h-4 mr-2" aria-hidden="true" viewBox="0 0 24 24" fill="currentColor">
...
import { required, email, minLength, helpers } from '@vuelidate/validators' //import Vuelidate rule ต่าง ๆ ที่ vuelidate มี
...
export default {
  ...
  data(){
    return { 
      ...
      formLogin: {  
        email: '', //สร้างตัวแปร email เอาไว้ผูกกับกับแบบฟอร์ม  
        password: ''
      }
    }
  },
